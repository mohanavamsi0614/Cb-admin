...
function Attd() {
  ...
  if (selectedBlock === 8) {
    ...
    filteredTeams = teams.slice(41, 81); 
  }
  ...
  return (
    <div className="min-h-screen bg-[url('./assets/try.svg')] bg-white py-10 px-2 font-sans flex flex-col items-center relative">
      
      {/* Back Button */}
      <button
        onClick={() => navigate("/")}
        className="absolute top-4 left-4 flex items-center gap-2 px-3 py-2 bg-black text-white hover:bg-gray-800 rounded-xl shadow-md transition"
      >
        <ArrowLeft size={20} />
        <span className="font-medium">Home</span>
      </button>

      <h1 className="text-black mt-10">{currAttd}</h1>
      ...
      {loading ? (
        <div className="text-center text-lg font-semibold text-cyan-300 animate-pulse">
          Loading teams...
        </div>
        ...
        <div className="flex flex-col gap-10 max-w-4xl w-full mx-auto">
          {filteredTeams.map((team, index) => (
            <div
              key={index}
              className="bg-white rounded-3xl shadow-2xl p-6 border border-black"
            >
